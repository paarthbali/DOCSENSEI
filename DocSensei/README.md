...
- Walk through the RAG pipeline in your own words: load → chunk → embed → store → retrieve → generate.
- Explain **why** two chunking strategies are compared: chunk size affects both retrieval relevance and cost — this is a real trade-off in production RAG systems, not just a toy feature.
- Explain the difference between the **local, free embedding step** (no API key) and the **remote LLM generation step** (needs an API key) — a common point of confusion worth clarifying.
- Demo the refusal behavior live by asking something not in the document.
- Show the modular file structure (`utils/`) and explain why RAG systems are usually built this way (single-responsibility, testable pieces) rather than one big script.
